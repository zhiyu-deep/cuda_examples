...
void MyGemmGlobalV3Repeats(float *a, float *b, float *c,
    ...
    MyGemmGlobalImplV3<tileH, tileW, tileK, 2, 2><<<grid, block>>>(a, b, c, M, N, K);
}
...
void MyGemmGlobalV3NoRepeats(float *a, float *b, float *c,
    ...
    dim3 block(256);
    dim3 grid(N / (16 * tileW * repeatsX), M / (16 * tileH * repeatsY));
    MyGemmGlobalImplV3<tileH, tileW, tileK, repeatsY, repeatsX><<<grid, block>>>(a, b, c, M, N, K);
}